package tp11;

import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;

import javax.swing.*;
import java.awt.*;

public class MaFenetre2 extends JFrame implements ActionListener{
	//ActionListener est une interface d'écoute.
	private static final long serialVersionUID = 1L;
	private JLabel labelSaisie1;
	private JLabel labelSaisie2;
	private JLabel labelResultat;
	private JTextField textFieldSaisie1;
	private JTextField textFieldSaisie2;
	private JTextField textFieldResultat;
	private JButton boutonSomme;
	private JButton boutonProduit;
	private JButton boutonQuitter;
	
	public MaFenetre2()
	{
		setSize(750,75); // Taille largeur et hauteur en pixels
		setTitle("Ma fenetre"); //Titre
		
		labelSaisie1 = new JLabel("SAISIE_1");
		labelSaisie2 = new JLabel("SAISIE_2");
		labelResultat = new JLabel("RESULTAT");
		
		textFieldSaisie1 = new JTextField(10);
		textFieldSaisie2 = new JTextField(10);
		textFieldResultat = new JTextField(10);
		
		boutonSomme = new JButton("+");
		boutonProduit = new JButton("*");
		boutonQuitter = new JButton("QUIT");
		
		getContentPane().setLayout(new FlowLayout());
		getContentPane().add(labelSaisie1);
		getContentPane().add(textFieldSaisie1);
		getContentPane().add(labelSaisie2);
		getContentPane().add(textFieldSaisie2);
		getContentPane().add(labelResultat);
		getContentPane().add(textFieldResultat);
		getContentPane().add(boutonSomme);
		getContentPane().add(boutonProduit);
		getContentPane().add(boutonQuitter);
		
		boutonSomme.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent arg0) {
				System.out.println("Calcul somme");
				int saisie1 = Integer.parseInt(textFieldSaisie1.getText());
				int saisie2 = Integer.parseInt(textFieldSaisie2.getText());
				int somme = saisie1 + saisie2;
				String strSomme = Integer.toString(somme);
				textFieldResultat.setText(strSomme);
			}
		});
		
		boutonProduit.addActionListener(new ActionListener() 
		{
			public void actionPerformed(ActionEvent arg0) {
				System.out.println("Calcul produit");
				int saisie1 = Integer.parseInt(textFieldSaisie1.getText());
				int saisie2 = Integer.parseInt(textFieldSaisie2.getText());
				int produit = saisie1 * saisie2;
				String strSomme = Integer.toString(produit);
				textFieldResultat.setText(strSomme);
			}
		});
		
		boutonQuitter.addActionListener(new CloseListener());
	}
	
	private class CloseListener implements ActionListener{
	    @Override
	    public void actionPerformed(ActionEvent e) {
	        //DO SOMETHING
	        System.exit(0);
	    }
	}

	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		
	}
}